console.log("Paginator.js: Module Evaluation Start");
    ...
    async load(src, data, afterLoad, beforeRender) {
        ...
        return new Promise(resolve => {
            this.#iframe.addEventListener('load', () => {
                console.log("InternalView: 1:1 READEST MATCH - LOAD FIRED!"); // KEEP THIS ONE TAG
            ...
            if (data) {
                // FORCE BLOB MODE DEBUGGING: Ignore srcdoc, use src (Blob URL)
                console.log("InternalView: IGNORING srcdoc (Data present). Forcing src (Blob URL) for debugging.");
                console.log("InternalView: Target Src:", src);
                // FORCE BLOB MODE: Ignore srcdoc, use src (Blob URL)
                // This bypasses potential CSP issues with srcdoc (inline html)
                console.log("InternalView: FORCING BLOB SRC (Ignoring srcdoc data). Src:", src);
                this.#iframe.src = src
...
export class Paginator extends HTMLElement {
    ...
    #observer = new ResizeObserver(() => {
        console.log("Paginator: ResizeObserver triggered");
        this.render();
    })
    #top
    #background
    ...
    constructor() {
        super()
        console.log("Paginator: Constructor called");
    ...
    open(book) {
        console.log("Paginator: open() called with book", book);
    ...
    async #goTo({ index, anchor, select }) {
        console.log("Paginator: #goTo called for index", index);
    ...
    async goTo(target) {
        console.log("Paginator: PUBLIC goTo called");
        if (this.#locked) {
            console.log("Paginator: PUBLIC goTo LOCKED");
            return;
        }
        const resolved = await target
        console.log("Paginator: PUBLIC goTo resolved target:", resolved);
        if (this.#canGoToIndex(resolved.index)) return this.#goTo(resolved)
        else console.log("Paginator: PUBLIC goTo cannot go to index:", resolved.index);
    }
    #scrollPrev(distance) {
